...
description: 'Give your agent a Telegram interface so you can chat with it from your phone.'
---

Your agent can run as a Telegram bot, giving you a mobile-friendly interface to your knowledge base, planning, and all agent capabilities. The Telegram transport runs alongside the [standard MCP interface](/docs/guides/transports/), so you can use both.
...
- `npm run telegram:start` for production mode
- `npm run telegram:dev` for development mode with auto-restart (via tsx)
...
- Environment variable: skip this step if you already set `ANTHROPIC_API_KEY` or `OPENAI_API_KEY` in your environment
...
Pick a default model. For Anthropic: Claude Sonnet 4 (fast, recommended), Claude Opus 4 (more capable), or Claude 3.5 Haiku (economical). For OpenAI: GPT-4.1, GPT-4.1-mini, or o3.
...
_Previous: [Getting Started](/docs/getting-started/). Next: [Customizing Your Agent](/docs/guides/customizing/). See also [Transports](/docs/guides/transports/) for other connection methods and the [CLI Reference](/docs/cli-reference/) for all `soleri telegram` subcommands._
