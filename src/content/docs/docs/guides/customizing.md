...
`_engine.md` is regenerated every time you run `soleri agent refresh`. If the engine rules change in a new Soleri version, your `_engine.md` updates automatically. `user.md` was added so your own rules survive that regeneration — they're source files, not generated output.
