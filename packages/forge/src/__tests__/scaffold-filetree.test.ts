...
describe('scaffoldFileTree', () => {
  ...
  it('does NOT generate user.md — it is an owner-override file, not a scaffold default', () => {
    ...
    expect(result.filesCreated).not.toContain('instructions/user.md');
  });

  it('CLAUDE.md still composes correctly when user.md is absent', () => {
    const result = scaffoldFileTree(MINIMAL_CONFIG, tempDir);
    expect(result.success).toBe(true);

    const claudeMd = readFileSync(join(result.agentDir, 'CLAUDE.md'), 'utf-8');
    // Engine rules ref must still be present
