...
const ENGINE_RULES_LINES: string[] = [
  ...
  '',
  '**Terse Mode** — When TERSE-MODE is active (via `op:morph` or SessionStart hook), enforce word budgets:',
  '- Three levels: lite (100 words max), full (60 words max, default), ultra (30 words max). Code blocks exempt from count.',
  '- full: No markdown headers, bullet lists, numbered lists. Dense prose or single code block. Core answer only — one cause, one fix.',
  '- Never restate question, add intros/conclusions, use filler/hedging/pleasantries.',
  ...
  '- `action`: "accepted" (result was useful) or "dismissed" (result was irrelevant)',
  '- `confidence`: 0.0–1.0 how relevant the result was to the task',
  '',
  '**Report bad intel — the negative path is mandatory.** When a consulted vault entry',
  'proves wrong, outdated, or harmful, call `op:brain_feedback` with:',
  '- `action`: "failed" (you followed the guidance and it was wrong) or "dismissed" (it did not apply)',
  '- `reason`: one line on what was wrong — this is what future rankings learn from',
  '',
  'Never silently ignore a bad vault entry. Pattern strengths only stay honest when',
  'failures are recorded; a vault that only hears "accepted" learns nothing.',
  '',
  'Do this for:',
  ...
  "- Results you browse but don't act on (the engine auto-dismisses unused search siblings)",
