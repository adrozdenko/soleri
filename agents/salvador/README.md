...
Salvador is your team's persistent knowledge layer. It:
...
The agent starts empty. That's intentional. Your expertise is the content; Salvador is the infrastructure that makes it searchable, persistent, and always available.
...
# 3. Start a new Codex session, then say:
#    "Hello, Salvador!"
```

That's it. Salvador will activate, check your setup, and offer to configure itself.

## Prerequisites

- **Node.js 18+** — `node --version` to check

## What Happens When You Say "Hello, Salvador!"

- Salvador returns its persona, principles, and tool recommendations
...
| ----------- | ----------------------------- |
| design      | _Ready for knowledge capture_ |
| component   | _Ready for knowledge capture_ |
| figma       | _Ready for knowledge capture_ |
| code-review | _Ready for knowledge capture_ |
...
Salvador ships with 17 structured workflow skills, available via generated AGENTS.md + local skill files in Codex:
...
**Model routing** (optional) — `~/.salvador/model-routing.json`:
...
- **"Hello, Salvador!"** — Activate the persona. Your active client session adopts Salvador's identity, principles, and tool access.
- **"Goodbye, Salvador!"** — Deactivate and return to default assistant behavior.

### Daily Workflow

Once active, Salvador works as your domain advisor. Here's how a typical session looks:

**Ask for guidance** — Salvador searches its vault and responds with relevant patterns:
\`\`\`
You: How should we handle rate limiting?
Salvador: [searches vault] Based on my patterns, here's what I recommend...
\`\`\`

**Capture knowledge** — When you discover something worth remembering, tell Salvador to capture it:
\`\`\`
You: We decided to use token bucket for rate limiting. Capture this as a pattern.
Salvador: [captures to vault with auto-tags: rate-limiting, token-bucket, api-design]
\`\`\`

**Plan multi-step work** — For larger tasks, Salvador creates trackable plans:
\`\`\`
You: Plan out the caching implementation.
Salvador: Created plan "Add Caching Layer" with 4 tasks: 1. Define cache invalidation strategy 2. Add Redis client configuration 3. Implement cache middleware 4. Add cache-related tests
Approve to start execution?
\`\`\`

**Give feedback** — When Salvador suggests something, your response teaches it:
\`\`\`
You: That pattern doesn't apply here — we're using GraphQL, not REST.
Salvador: [records feedback] Got it. I'll adjust future recommendations.
\`\`\`
...
Salvador: [searches vault + memory] You chose token bucket with...
\`\`\`
...
Salvador starts with an empty vault. Here's how to fill it with knowledge.

### One at a Time (conversational capture)

The simplest way — tell Salvador what you've learned and ask it to capture:

\`\`\`
You: We decided to use token bucket for rate limiting with a 100 req/min limit.
Capture this as a pattern in api-design.

Salvador: [captures to vault]
...
To populate the vault from existing documents (architecture docs, coding standards, RFCs, post-mortems), paste or reference the document and ask Salvador to extract patterns:
...
Salvador: [reads document, extracts and captures each pattern]
Captured 12 entries: - api-auth-jwt (pattern, critical) — Always use short-lived JWTs - api-no-sql-in-controllers (anti-pattern, warning) — Keep SQL in data layer - api-pagination-required (rule, warning) — All list endpoints must paginate
...
\`\`\`
...
Salvador gets smarter through use — no code changes needed:
...
4. **Register projects** — Register Salvador with multiple projects. It builds context about each one and surfaces relevant knowledge on activation.
5. **Export and share** — Export vault knowledge as JSON. Share bundles with teammates or back them up in version control.

The more you interact, the more valuable Salvador becomes. A fresh agent with 50+ captured patterns and a month of feedback is significantly more useful than one on day one.

## Roadmap

Salvador ships with a solid foundation. Below are optional improvements you can add yourself, ordered by impact.
...
**Why:** Currently the agent responds to explicit ops. A context engine lets it understand _what the user means_ and proactively retrieve relevant knowledge.
