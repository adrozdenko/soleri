...
describe('E2E: vault-zettelkasten', () => {
  ...
  describe('Journey 1: Knowledge capture + search cycle', () => {
    ...
    it('should capture a pattern entry with full metadata', async () => {
      entryIdA = await captureEntry({
        ...
        description:
          'Use custom hooks to extract reusable stateful logic from components. Prefer useCallback and useMemo for expensive computations.',
    ...
    it('should capture a second entry in a different category', async () => {
      entryIdB = await captureEntry({
        ...
        description:
          'Create indexes on columns used in WHERE, JOIN, and ORDER BY clauses. Avoid over-indexing write-heavy tables.',
    ...
    it('should find both entries via domain-specific and general search', async () => {
      const frontendRes = await callOp(vaultFacade, 'search', {
      ...
      const backendRes = await callOp(vaultFacade, 'search', {
  ...
  describe('Journey 2: Zettelkasten linking', () => {
    ...
    beforeAll(async () => {
      idA = await captureEntry({
        ...
        description:
          'Break monolith into independently deployable services communicating via APIs.',
      ...
      idB = await captureEntry({
        ...
        description:
          'Use a sidecar proxy to handle service-to-service communication, retries, and observability.',
      ...
      idC = await captureEntry({
        ...
        description:
          'Start with a monolith and extract services only when complexity demands it. Premature decomposition is costly.',
    ...
    it('get_links for A should show outgoing to B and incoming contradicts from C', async () => {
      ...
      expect(data.incoming.some((l) => l.sourceId === idC && l.linkType === 'contradicts')).toBe(
        true,
      );
    });
  });
  ...
  describe('Journey 3: Graph traversal', () => {
    ...
    it('traverse from A with depth 3 should find B, C, and D', async () => {
      ...
      expect(res.success).toBe(true);
      const data = res.data as {
        connectedEntries: Array<{ id: string; linkType: string }>;
  ...
  describe('Journey 4: Orphan detection', () => {
    ...
    it('linking one orphan should reduce orphan count', async () => {
      ...
      const afterOrphanIds = (afterData as { orphans: Array<{ id: string }> }).orphans.map(
        (o) => o.id,
      );
  ...
  describe('Journey 5: Link suggestions', () => {
    ...
    beforeAll(async () => {
      hooksId = await captureEntry({
        ...
        description:
          'Build reusable React custom hooks for data fetching, form validation, and state management.',
      ...
      stateId = await captureEntry({
        ...
        description:
          'Compare React state management approaches: useState, useReducer, Context, Zustand, Redux.',
      ...
      perfId = await captureEntry({
        ...
        description:
          'Optimize React rendering with memo, useMemo, useCallback, and code splitting.',
        ...
        entries: [
          {
            ...
            description:
              'Test React components that use hooks with React Testing Library. Mock custom hooks for isolation.',
  ...
  describe('Edge cases', () => {
    ...
    it('very long description should handle without truncation issues', async () => {
      ...
      const res = await callOp(vaultFacade, 'search', {
        query: 'Entry With Very Long Description',
      });
      expect(res.success).toBe(true);
      const results = res.data as Array<{ entry: { id: string } }>;
