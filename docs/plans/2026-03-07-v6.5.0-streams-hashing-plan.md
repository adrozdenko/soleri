...
describe('computeContentHash', () => {
  ...
  it('normalizes whitespace', () => {
    ...
    const b = computeContentHash({ ...base, title: '  Test Pattern  ', description: '  A test pattern.  ' });
    expect(a).toBe(b);
  });
...
### Task 5: Add content\_hash column and backfill to Vault
...
In `initialize()`, after the existing indexes block (after `idx_memories_type`), add the content\_hash migration:
...
```typescript
    ...
    const unhashed = this.provider.all<{ id: string; type: string; domain: string; title: string; description: string; tags: string; example: string | null; counter_example: string | null }>(
    ...
    if (unhashed.length > 0) {
      this.provider.transaction(() => {
        for (const row of unhashed) {
          ...
          this.provider.run('UPDATE entries SET content_hash = @hash WHERE id = @id', { hash, id: row.id });
        }
      });
    }
```
...
```typescript
  describe('Content-addressable hashing', () => {
    it('seed populates content_hash', () => {
      vault.seed([{ id: 'ch-1', type: 'pattern', domain: 'test', title: 'Hash test', severity: 'warning', description: 'Desc', tags: ['a'] }]);
      const row = vault.get('ch-1') as Record<string, unknown>;
      expect(row).not.toBeNull();
      // Verify hash is set via findByContentHash
      const hash = computeContentHash({ type: 'pattern', domain: 'test', title: 'Hash test', description: 'Desc', tags: ['a'] });
    ...
    it('contentHashStats returns correct counts', () => {
      vault.seed([
        { id: 'hs-1', type: 'pattern', domain: 'd', title: 'T1', severity: 'warning', description: 'D1', tags: ['a'] },
        { id: 'hs-2', type: 'rule', domain: 'd', title: 'T2', severity: 'warning', description: 'D2', tags: ['b'] },
      ]);
...
### Task 7: Add vault\_content\_hash and vault\_dedup\_status ops
